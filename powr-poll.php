<?php
    ...
    Plugin URI: https://www.powr.io/plugins/poll
    ...
    */
    ...
    class Powr_Poll extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Poll.
        </p>
        <p>
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_poll_menu' );
    function powr_poll_menu() {
      add_menu_page( 'POWr Poll', 'POWr Poll', 'manage_options', 'powr-poll-settings', 'powr_poll_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_poll_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/poll';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/poll';
      }
    ...
    register_activation_hook(__FILE__, 'powr_poll_plugin_activate');
      add_action('admin_init', 'powr_poll_plugin_redirect');

      function powr_poll_plugin_activate() {
      add_option('powr_poll_plugin_do_activation_redirect', true);
      }
      ...
      function powr_poll_plugin_redirect() {
      if (get_option('powr_poll_plugin_do_activation_redirect', false)) {
          delete_option('powr_poll_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-poll-settings&' );
          }
       }
      }
